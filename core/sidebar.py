import pygame

class SideBar:
    def __init__(self, screen_w, screen_h, width=120):
        self.width = width
        self.rect = pygame.Rect(screen_w - width, 0, width, screen_h)
        self.elements = ["kotilum","gejimtium"]
        self.grid_size = 100
        self.padding = 10
        self.scroll_offset = 0

    def discover(self, element_name):
        if element_name not in self.elements:
            self.elements.append(element_name)

    def draw(self, screen):
        pygame.draw.rect(screen, (230, 230, 230), self.rect)

        # Clip drawing to sidebar only
        clip = screen.get_clip()
        screen.set_clip(self.rect)

        for i, name in enumerate(self.elements):
            y = self.padding + i * (self.grid_size + self.padding) + self.scroll_offset
            try:
                image = pygame.image.load(f"assets/{name}.png").convert_alpha()
                image = pygame.transform.scale(image, (self.grid_size, self.grid_size))
                screen.blit(image, (self.rect.x + 10, y))
            except Exception:
                continue

        screen.set_clip(clip)

    def handle_click(self, pos):
        if not self.rect.collidepoint(pos):
            return None

        for i, name in enumerate(self.elements):
            y = self.padding + i * (self.grid_size + self.padding) + self.scroll_offset
            rect = pygame.Rect(self.rect.x + 10, y, self.grid_size, self.grid_size)
            if rect.collidepoint(pos):
                return name
        return None

    def handle_scroll(self, event):
        if self.rect.collidepoint(pygame.mouse.get_pos()):
            if event.y > 0:
                self.scroll_offset = min(self.scroll_offset + 30, 0)
            elif event.y < 0:
                max_scroll = max(0, len(self.elements) * (self.grid_size + self.padding) - self.rect.height)
                self.scroll_offset = max(self.scroll_offset - 30, -max_scroll)
